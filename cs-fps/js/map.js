...
function makeBox(scene, x, y, z, w, h, d, color, collide) {
...
function createMap(scene) {
  ...
  makeBox(scene, 0, 0, -W - T / 2, W * 2 + T * 2, PH, T, 0x75777e);
  makeBox(scene, 0, 0,  W + T / 2, W * 2 + T * 2, PH, T, 0x75777e);
  makeBox(scene, -W - T / 2, 0, 0, T, PH, W * 2, 0x75777e);
  makeBox(scene,  W + T / 2, 0, 0, T, PH, W * 2, 0x75777e);

  /* ---- interior walls (gray, h=4) ---- */
  const H = 4, WC = 0x82848c;
  // lane dividers west/east (leave an 8u mid-west / mid-east gap)
  makeBox(scene, -12, 0, -18, 1, H, 20, WC);   // z -28..-8
  makeBox(scene, -12, 0,  18, 1, H, 20, WC);   // z   8..28
  makeBox(scene,  12, 0, -18, 1, H, 20, WC);
  makeBox(scene,  12, 0,  18, 1, H, 20, WC);
  // site south walls with 4u doorways at x=±14
  makeBox(scene, -25, 0, -10, 18, H, 1, WC);   // x -34..-16
  makeBox(scene,  25, 0, -10, 18, H, 1, WC);   // x  16..34
  // mid cover chunks
  makeBox(scene, 0, 0,   8, 8, H, 1, WC);      // blocks spawn-to-spawn sightline
  makeBox(scene, 0, 0, -20, 6, H, 1, WC);      // mid-north block
  ...
  for (const [cx, cz, stacked] of crateSpots) {
    makeBox(scene, cx, 0, cz, S, S, S, CRATE);
    if (stacked) makeBox(scene, cx, S, cz, S, S, S, CRATE2, false);
  }
