...
/* colorful character roster — each round 5 random characters fight you */
const BOT_CHARACTERS = [
  { name: 'Rex',   color: 0xe74c3c }, { name: 'Nova',  color: 0x3498db },
  { name: 'Blaze', color: 0xe67e22 }, { name: 'Zap',   color: 0xf1c40f },
  { name: 'Pip',   color: 0x2ecc71 }, { name: 'Juno',  color: 0x9b59b6 },
  { name: 'Koda',  color: 0x1abc9c }, { name: 'Vex',   color: 0xfd79a8 },
  { name: 'Skye',  color: 0x00cec9 }, { name: 'Moss',  color: 0x8bc34a }
];
/* 3 bosses with increasing difficulty (cycled every 4th round, stats scale up each cycle) */
const BOSS_DEFS = [
  { name: 'BRUTUS', hp: 300, scale: 1.55, speed: 3.0, dmgMult: 1.5, fireRateMult: 0.8,
    strafeScale: 1, color: 0xd4af37, gun: 'rifle', special: null },
  { name: 'VIPER', hp: 500, scale: 1.75, speed: 4.8, dmgMult: 1.7, fireRateMult: 0.55,
    strafeScale: 0.45, color: 0x9b59b6, gun: 'rifle', special: null },
  { name: 'TITAN', hp: 800, scale: 2.2, speed: 2.7, dmgMult: 2.0, fireRateMult: 1.0,
    strafeScale: 1, color: 0x34495e, gun: 'rifle', special: 'orbs' }
];
const bots = [];
...
class Bot {
  constructor(id, scene, spawn, difficulty, character) {
    this.id = id;
    this.character = character || BOT_CHARACTERS[id % BOT_CHARACTERS.length];
    ...
    this.isBoss = false;
    this.scaleF = 1;
    this.dmgMult = 1;        // boss damage multiplier
    this.fireRateMult = 1;   // boss fire-rate multiplier (<1 = faster)
    this.strafeScale = 1;    // boss strafe timer multiplier (<1 = twitchier)
    this.state = 'patrol';
    ...
    this.deathT = 0;
    this.flashLeft = 0;
    this.special = null;     // boss special attack ('orbs')
  ...
  _build(scene, spawn) {
    ...
    // character-colored outfit (clone materials so bots stay colorful)
    const torsoMat = new THREE.MeshLambertMaterial({ color: this.character.color });
    const headMat = new THREE.MeshLambertMaterial({
      color: new THREE.Color(this.character.color).offsetHSL(0, -0.15, 0.25) });
    ...
    this.torso = new THREE.Mesh(_geoTorso, torsoMat); this.torso.position.set(0, 1.275, 0);
    this.head = new THREE.Mesh(_geoHead, headMat); this.head.position.set(0, 1.89, 0);
  ...
  _shoot(now) {
    // TITAN lobs explosive orbs instead of hitscan fire
    if (this.special === 'orbs') { this._shootOrb(now); return; }
    ...
    this.nextShotAt = now + gun.interval * this.fireRateMult;
    this.burstLeft--;
    if (this.burstLeft <= 0) {
      const p = this.diff.burstPause;
      this.nextBurstAt = now + (p[0] + Math.random() * (p[1] - p[0])) * this.fireRateMult;
    }
    ...
    if (t > 0 && t < wallDist) {
      ...
      if (closest.distanceTo(_v2) < 0.62) {
        const dmg = gun.damage * this.diff.dmgMult * this.dmgMult * (0.85 + Math.random() * 0.3);
  ...
  /* TITAN special: slow explosive orb aimed at the player (dodgeable) */
  _shootOrb(now) {
    if (now < this.nextShotAt) return;
    this.nextShotAt = now + 1.7 / this.fireRateMult;
    _v1.copy(this.pos); _v1.y = 1.35 * this.scaleF;
    _v2.copy(player.pos); _v2.y -= 0.3;
    const dist = _v1.distanceTo(_v2);
    // slight lead on the moving player
    _v2.x += player.vel.x * dist / 28 * 0.5;
    _v2.z += player.vel.z * dist / 28 * 0.5;
    const dir = _v2.sub(_v1).normalize();
    spawnProjectile(_v1.clone(), dir,
      { projectile: { speed: 15, gravity: 0, type: 'orb' } }, true);
    SFX.gunshot('whoosh', Math.max(0.25, 1 - dist / 60));
    this.flash.visible = true;
    this.flashLeft = 0.08;
  }
  ...
  update(dt, now) {
    ...
    if (this.state === 'patrol') {
      ...
      if (this.seesPlayer) {
        ...
        this.strafeT -= dt;
        if (this.strafeT <= 0) {
          this.strafeT = (1 + Math.random()) * this.strafeScale;
          this.strafeDir *= -1;
        }
...
class Boss extends Bot {
  constructor(scene, spawn, difficulty, tier, statScale) {
    const def = BOSS_DEFS[tier % BOSS_DEFS.length];
    super(0, scene, spawn, difficulty, { name: def.name, color: def.color });
    this.isBoss = true;
    this.bossDef = def;
    this.maxHp = Math.round(def.hp * statScale);
    this.hp = this.maxHp;
    this.scaleF = def.scale;
    this.speed = def.speed;
    this.dmgMult = def.dmgMult * statScale;
    this.fireRateMult = def.fireRateMult;
    this.strafeScale = def.strafeScale;
    this.special = def.special;
    this.gun = def.gun;
    this.group.scale.set(def.scale, def.scale, def.scale);

    // golden crown so you know it's royalty
    const crown = new THREE.Mesh(new THREE.BoxGeometry(0.42, 0.14, 0.42),
      new THREE.MeshLambertMaterial({ color: 0xffd700 }));
    crown.position.set(0, 2.16, 0);
    this.group.add(crown);
    const spike = new THREE.Mesh(new THREE.BoxGeometry(0.1, 0.14, 0.1),
      new THREE.MeshLambertMaterial({ color: 0xffd700 }));
    spike.position.set(0, 2.28, 0);
    this.group.add(spike);

    this.hpBar.scale.set(1.1, 0.14, 1);   // scales with the group
    this._drawHp();
  }

  takeDamage(dmg, headshot) {
    super.takeDamage(dmg, headshot);
    if (this.alive) ui.setBossBar(this.hp / this.maxHp);
  }
...
function spawnBots(scene, count, difficulty) {
  clearBots(scene);
  const roster = BOT_CHARACTERS.slice();
  // shuffle so every round has a different cast
  for (let i = roster.length - 1; i > 0; i--) {
    const j = (Math.random() * (i + 1)) | 0;
    [roster[i], roster[j]] = [roster[j], roster[i]];
  }
  for (let i = 0; i < count; i++) {
    bots.push(new Bot(i, scene, botSpawns[i % botSpawns.length], difficulty, roster[i]));
  }
}

/* boss round: one boss + two minion bodyguards */
function spawnBossRound(scene, difficulty, tier, statScale) {
  clearBots(scene);
  const boss = new Boss(scene, new THREE.Vector3(0, 0, -34), difficulty, tier, statScale);
  bots.push(boss);
  const roster = BOT_CHARACTERS.filter(c => c.name !== boss.name);
  for (let i = 0; i < 2; i++) {
    const m = new Bot(i + 1, scene, botSpawns[i * 4], difficulty, roster[i]);
    m.hp = 120; m.maxHp = 120; m._drawHp();
    bots.push(m);
  }
  return boss;
}
