...
/* ---------------- mobile input state ---------------- */
const mobileMove = { x: 0, y: 0 };   // left pad: x strafe, y forward
const mobileLook = { x: 0, y: 0 };   // right pad: x yaw rate, y pitch rate
let mobileYaw = 0, mobilePitch = 0;  // camera accumulators for touch look
const _meu = new THREE.Euler(0, 0, 0, 'YXZ');
...
class Player {
  ...
  _bindInput() {
    ...
    document.addEventListener('mousedown', (e) => {
      if (e.button !== 0 || game.mobile) return;   // mobile uses touch handlers
  ...
  update(dt) {
    ...
    // touch look: right pad drives yaw/pitch rate, camera set from accumulators
    if (game.mobile && this.alive) {
      mobileYaw -= mobileLook.x * 2.6 * dt;
      mobilePitch -= mobileLook.y * 2.6 * dt;
      mobilePitch = Math.max(-1.5, Math.min(1.5, mobilePitch));
      _meu.set(mobilePitch, mobileYaw, 0);
      camera.quaternion.setFromEuler(_meu);
    }
    ...
    let fx = 0, fz = 0;
    if (game.mobile) {
      fx = mobileMove.x;              // left pad: strafe + forward/back
      fz = mobileMove.y;
    } else {
      if (k['KeyW']) fz += 1;
      if (k['KeyS']) fz -= 1;
